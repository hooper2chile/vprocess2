...
logging.basicConfig(filename='/home/hooper/vprocess2/log/database.log', level=logging.INFO, format='%(asctime)s:%(levelname)s:%(message)s')
TIME_MIN_BD = 1 # 1 [s]
...
def update_db(real_data, connector, c, first_time, BACKUP):
    ...
    if BACKUP:
        ...
        filedb='/home/hooper/vprocess2/database/backup__' + first_time + '__.db'

        bck = sqlite3.connect(filedb)
        sqlitebck.copy(connector, bck)

        try:
            os.system('sqlite3 -header -csv %s "select * from ph;"   > /home/hooper/vprocess2/csv/%s' % (filedb,filedb[31:-3])+'full_ph.csv' )
            os.system('sqlite3 -header -csv %s "select * from od;"   > /home/hooper/vprocess2/csv/%s' % (filedb,filedb[31:-3])+'full_od.csv' )
            os.system('sqlite3 -header -csv %s "select * from temp;" > /home/hooper/vprocess2/csv/%s' % (filedb,filedb[31:-3])+'full_temp.csv' )
